...
function custom_comment_form_fields($fields)
{
    ...
    $fields = array(
        'author' => '<div class="row"><div class="col-xl-6 col-lg-6 col-12 mb-20"><div class="tp-contact-form-input-box">' .
            '<input type="text" name="author" id="author" placeholder="' . esc_attr__('Name*', 'textdomain') . '" value="' . esc_attr($commenter['comment_author']) . '" ' . ($req ? 'required' : '') . '></div></div>',

        'email' => '<div class="col-xl-6 col-lg-6 col-12 mb-20"><div class="tp-contact-form-input-box">' .
            '<input type="email" name="email" id="email" placeholder="' . esc_attr__('Email', 'textdomain') . '" value="' . esc_attr($commenter['comment_author_email']) . '" ' . ($req ? 'required' : '') . '></div></div>',

        'url' => '<div class="col-xl-12 col-lg-12 col-12 mb-20"><div class="tp-contact-form-input-box">' .
            '<input type="text" name="url" id="url" placeholder="' . esc_attr__('Website', 'textdomain') . '" value="' . esc_attr($commenter['comment_author_url']) . '"></div></div></div>',
    );
...
function custom_comment_form_comment($comment_field)
{
    $comment_field = '<div class="row"><div class="col-12 mb-20"><div class="tp-contact-form-input-box">' .
        '<textarea id="comment" name="comment" placeholder="' . esc_attr__('Your Comment Here...', 'textdomain') . '" required></textarea></div></div> </div>';
...
{
    $submit_button = '<div class="crank-comments-btn">' .
        '<button type="submit" class="tp-btn">' . esc_html__('Submit Comment', 'textdomain') . '</button></div>';
...
?>
        <li class="pingback">
            <p><?php esc_html_e('Pingback:', 'crank'); ?> <?php comment_author_link(); ?></p>
        </li>
    <?php
    } else {
                    ...
                    <div class="postbox__comment-name p-relative">
                        ...
                        <div class="postbox__comment-text">
                            <?php if ($comment->comment_approved == '0') : ?>
                                <p><?php esc_html_e('Your comment is awaiting moderation.', 'crank'); ?></p>
                            <?php endif; ?>
                            <?php comment_text(); ?>
